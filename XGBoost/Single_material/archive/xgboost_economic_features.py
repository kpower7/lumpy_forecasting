#%% [markdown]
# XGBoost Model with Extended Economic Indicators
# This version includes longer timeframes for economic variables and feature selection
...
def create_features(df):
    ...
    # Extended lags for economic indicators (up to 24 months)
    for col in economic_vars:
        for i in range(1, 25):  # 24 months of lags
            df[f'{col}_lag_{i}'] = df[col].shift(i)
    
    # Regular lags for market indicators (up to 12 months)
...
def train_and_evaluate(material_id, material_name):
    ...
    feature_columns = [col for col in train_data.columns 
                      if any(x in col for x in ['lag_', 'roll_', 'Year', 'Month', 'Quarter']) 
                      or col in ['ECG_DESP', 'TUAV', 'PIB_CO', 'ISE_CO', 'VTOTAL_19', 'OTOTAL_19', 'ICI']]
    ...
    print(f"\nFeatures selected: {len(selected_features)} out of {len(feature_columns)}")
