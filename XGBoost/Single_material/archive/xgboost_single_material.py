...
#%%
# Use data up to August 2024 for training (since we have missing external variables after September)
train = train_data[train_data['YearMonth'] < '2024-09-01']
test = train_data[train_data['YearMonth'] >= '2024-09-01']
...
feature_columns.extend([col for col in train.columns if 'lag' in col or 'roll_mean' in col])
...
results = pd.DataFrame({
    'Date': train_data['YearMonth'],
    'Actual': train_data['Dispatched Quantity'],
...
plt.axvline(x=pd.to_datetime('2024-09-01'), color='r', linestyle='--', label='Train/Test Split')
