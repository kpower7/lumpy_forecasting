...
def train_and_evaluate(material_id, material_name):
    ...
    dates = pd.concat([train['YearMonth'], test['YearMonth']])
    ...
    ax = category_importance.plot(kind='bar')
    plt.title(f'Feature Importance by Category ({material_name})')
    plt.xlabel('Feature Category')
    plt.ylabel('Total Importance')
    plt.xticks(range(len(category_importance.index)), category_importance.index, rotation=45, ha='right')
    plt.tight_layout()
    plt.show()
...
#%%
# Analyze Material 1
metrics_1, importance_1 = train_and_evaluate('555136414', 'Material_1')

# Analyze Material 21
metrics_21, importance_21 = train_and_evaluate('333180636', 'Material_21')

# Analyze Material 51
metrics_51, importance_51 = train_and_evaluate('0001O1010', 'Material_51')

# Analyze Material 56
metrics_56, importance_56 = train_and_evaluate('0001O1184', 'Material_56')

# Analyze Material 77
metrics_77, importance_77 = train_and_evaluate('0001O3160', 'Material_77')

# Analyze Material 82
metrics_82, importance_82 = train_and_evaluate('0001PZ701', 'Material_82')
...
comparison = pd.DataFrame({
    'Material_1': pd.Series(metrics_1),
    'Material_21': pd.Series(metrics_21),
    'Material_51': pd.Series(metrics_51),
    'Material_56': pd.Series(metrics_56),
    'Material_77': pd.Series(metrics_77),
    'Material_82': pd.Series(metrics_82)
})
