    ...
    def fit_transform(self, df, is_training=True):
        ...
        for col in market_cols:
            ...
            for window in [3, 6]:
                ...
                # Momentum calculation using lagged values to prevent leakage
                features[f'{col}_momentum_{window}'] = df[col].shift(1) - roll_mean
        ...
        for col in econ_cols:
            ...
            if is_training:
                roll_obj = df[col].rolling(window=6, min_periods=1)
            else:
                roll_obj = df[col].expanding(min_periods=1)
            features[f'{col}_roll_mean_6'] = roll_obj.mean()
...
def train_hybrid_model(material_id, material_name):
    ...
    plt.title(f'Hybrid Model v2.1: {material_name}')
    plt.xlabel('Date')
    plt.ylabel('Quantity')
    plt.legend()
