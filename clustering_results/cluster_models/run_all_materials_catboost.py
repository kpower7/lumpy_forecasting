...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('material_processing_catboost.log'),
...
forecast_script = r"C:\Users\k_pow\OneDrive\Documents\MIT\MITx SCM\IAP 2025\SCC\clustering_results\cluster_models\demand_forecast_catboost_new4.py"
...
def create_temp_script(material_id, material_name, original_script_path):
    ...
    temp_script_path = 'temp_forecast_script_catboost.py'
...
for idx, row in tqdm(materials_list.iterrows(), total=len(materials_list)):
    ...
    try:
        ...
        intermediate_path = os.path.join(os.path.dirname(forecast_script), 'material_r2_results_catboost_intermediate.csv')
...
output_path = os.path.join(os.path.dirname(forecast_script), 'material_r2_results_catboost.csv')
