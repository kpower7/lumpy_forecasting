...
def train_and_evaluate_material(material_id, cluster_num, data_dir):
    ...
    try:
        ...
        # Use time-series cross-validation for model validation
        n_splits = min(5, len(X_train) // 10)  # Ensure enough data per split
        cv_scores = []
        
        if n_splits >= 2:
            for i in range(n_splits):
                split_idx = len(X_train) - (i + 1) * len(X_test)
                if split_idx < 10:  # Ensure enough training data
                    break
                    
                X_cv_train = X_train.iloc[:split_idx]
                y_cv_train = y_train.iloc[:split_idx]
                X_cv_val = X_train.iloc[split_idx:split_idx + len(X_test)]
                y_cv_val = y_train.iloc[split_idx:split_idx + len(X_test)]
                
                if len(X_cv_train) < 5 or len(X_cv_val) < 5:
                    continue
                    
                # Train on CV split
                cv_model = get_optimized_model(cluster_num)
        ...
        metrics = {
            ...
            'CV_Score': np.mean(cv_scores) if cv_scores else None
        }
