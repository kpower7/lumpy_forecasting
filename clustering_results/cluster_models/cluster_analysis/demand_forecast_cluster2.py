...
def prepare_features(data, external_data, is_training=True):
    """
    Prepare features optimized for Cluster 2 - Low volume, intermittent demand
    """
    features = pd.DataFrame(index=data.index)
    
    # Demand patterns focusing on recent history
    for lag in [1, 2, 3, 7, 14]:  # Shorter lags for intermittent patterns
    ...
    for window in [7, 14, 30]:
        ...
        features[f'Zero_Demand_Days_{window}d'] = roll.apply(lambda x: (x == 0).sum())
    ...
    for lag in [1, 2, 3, 7, 14]:
        ...
        # Binary indicators for non-zero demand
        features[f'Had_Demand_Lag{lag}'] = (data['Dispatched Quantity'].shift(lag) > 0).astype(int)
...
def train_model(X_train, y_train, X_val, y_val):
    ...
    Parameters tuned specifically for sparse, low-volume patterns
    """
    model = XGBRegressor(
