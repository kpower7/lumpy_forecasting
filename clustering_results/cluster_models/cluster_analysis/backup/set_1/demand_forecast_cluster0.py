# %% [markdown]
# # Enhanced Demand Forecasting using XGBoost for Cluster 0 (Stable Mid-Volume Products)
# This script is optimized for products with stable demand patterns and positive economic sensitivity.
...
def prepare_features(data, monthly_data, is_training=True):
    ...
    # Enhanced demand lags for stable patterns
    for lag in [1, 2, 3, 7, 14, 30, 60]:  # Added longer lags for stable patterns
        features[f'Demand_lag_{lag}d'] = data['Dispatched Quantity'].shift(lag)

    # Rolling statistics with longer windows for stability
    for window in [7, 14, 30, 60, 90]:  # Added longer windows
        ...
        features[f'Demand_roll_cv_{window}d'] = roll.std() / roll.mean()
    
    # Enhanced economic indicators (important for Cluster 0)
    external_vars = ['ECG_DESP', 'TUAV', 'PIB_CO', 'ISE_CO', 'VTOTAL_19', 'OTOTAL_19', 'ICI']
    for var in external_vars:
        if var in monthly_data.columns:
            for lag in range(1, 4):
                lagged = monthly_data[var].shift(lag) if is_training else monthly_data[var].shift(lag + 1)
                features[f'{var}_lag_{lag}m'] = lagged.reindex(data.index).fillna(method='ffill', limit=30)
            
            # Add rolling means of economic indicators
            roll = monthly_data[var].rolling(window=3, min_periods=1)
            features[f'{var}_roll_mean_3m'] = roll.mean().reindex(data.index).fillna(method='ffill', limit=30)
...
def train_model(X_train, y_train, X_val, y_val):
    """
    Train XGBoost model optimized for Cluster 0
    """
    model = xgb.XGBRegressor(
        objective='reg:squarederror',
        n_estimators=200,  # Increased for stable patterns
