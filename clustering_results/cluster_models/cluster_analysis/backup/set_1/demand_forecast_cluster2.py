# %% [markdown]
# # Enhanced Demand Forecasting using XGBoost for Cluster 2 (Low-Volume Products)
# This script is optimized for products with low, intermittent demand patterns.
...
def prepare_features(data, monthly_data, is_training=True):
    """
    Prepare features optimized for Cluster 2 - emphasizing order patterns and basic indicators
    """
    features = pd.DataFrame(index=data.index)
    
    # Basic demand patterns with focus on recent history
    for lag in [1, 2, 3, 7]:  # Short lags for low-volume patterns
        features[f'Demand_lag_{lag}d'] = data['Dispatched Quantity'].shift(lag)
    
    # Order-based features (critical for low-volume products)
    if 'Customer Order Quantity' in data.columns:
        for lag in [1, 2, 3, 7]:
            features[f'Order_lag_{lag}d'] = data['Customer Order Quantity'].shift(lag)
            # Order vs Demand ratio
            ratio = (data['Customer Order Quantity'].shift(lag) / 
                    (data['Dispatched Quantity'].shift(lag) + 1e-6))  # Add small epsilon to avoid division by zero
            features[f'Order_Demand_Ratio_{lag}d'] = ratio.replace([np.inf, -np.inf], np.nan)
    
    # Rolling windows focused on demand occurrence rather than volume
    for window in [7, 14, 30]:
        # Count of non-zero demand days
        features[f'NonZero_Demand_{window}d'] = (data['Dispatched Quantity'] > 0).rolling(window).sum()
        if 'Customer Order Quantity' in data.columns:
            # Count of non-zero order days
            features[f'NonZero_Orders_{window}d'] = (data['Customer Order Quantity'] > 0).rolling(window).sum()
    
    # Basic economic indicators (monthly features)
    if monthly_data is not None:
        basic_vars = ['PIB_CO', 'ICI']  # Focus on basic economic indicators
    ...
    # Binary indicator for demand occurrence
    features['Had_Demand_Previous'] = (data['Dispatched Quantity'].shift(1) > 0).astype(int)
    features['Had_Order_Previous'] = (data['Customer Order Quantity'].shift(1) > 0).astype(int)
...
def train_model(X_train, y_train, X_val, y_val):
    """
    Train XGBoost model optimized for Cluster 2 - Low Volume
    """
...
if __name__ == "__main__":
    ...
    material_id = '000161032'  # Replace with actual material ID from Cluster 0
