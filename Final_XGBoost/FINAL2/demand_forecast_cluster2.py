# %% [markdown]
# # Enhanced Demand Forecasting using XGBoost for Cluster 2 (High-Demand Market-Sensitive Materials)
...
def train_model(X_train, y_train, X_val, y_val):
    """
    Train XGBoost model optimized for Cluster 2 - High-Demand Market-Sensitive Materials
    
    Tuned for:
    - High volume (avg 1,364 units)
    - High variability (std 940)
    - Strong market and economic sensitivity
    """
    model = xgb.XGBRegressor(
        objective='reg:squarederror',
        n_estimators=100,  # Reduced to prevent overfitting on sparse data
        learning_rate=0.01,  # Lower learning rate for stability with sparse data
        max_depth=4,  # Reduced depth to prevent overfitting
        min_child_weight=5,  # Increased to handle sparse data better
        subsample=0.7,  # Reduced to prevent overfitting
        colsample_bytree=0.7,  # Reduced to focus on most important features
        random_state=42
    )
