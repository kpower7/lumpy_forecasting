# %% [markdown]
# # Enhanced Demand Forecasting using CatBoost for Cluster 2 (High-Demand Market-Sensitive Materials)
# This script is optimized for high-volume materials with strong market and economic sensitivity.

# %% [markdown]
"""
Cluster 2: High-Demand Market-Sensitive Materials
20 materials (24% of total)

Characteristics:
- Highest average monthly demand (1,364 units)
- Highest absolute demand variability (std = 940)
- Moderate relative variability (CV = 0.57)
- Highest order fulfillment rate
- Strong positive correlation with all indicators:
  * Strong PIB sensitivity (0.43)
  * Positive ICI sensitivity (0.14)
  * Strong market sensitivity (ECG: 0.35, ISE: 0.51)

This cluster represents high-volume materials that are highly sensitive to both
economic and market conditions. These materials show the strongest positive
correlations with economic indicators, suggesting they are likely core business
materials that reflect overall market health.

Strategy:
- Implement sophisticated forecasting using all economic indicators
- Maintain higher safety stocks due to high volume and variability
- Use market indicators (especially ISE) as leading demand signals
- Focus on optimizing order fulfillment process for large volumes
"""
...
def load_data(material_id: str, material_name: str) -> pd.DataFrame:
    """
    Load and prepare data for a specific material in Cluster 2 (High-Demand Market-Sensitive)
    ...
    Returns:
    - DataFrame with prepared data for forecasting
    
    Notes:
    - Optimized for high-volume materials with strong market sensitivity
    - Includes all economic indicators due to strong correlations
    - Special attention to market indicators (ECG, ISE) as leading signals
    """
    try:
        # Load daily data
...
def prepare_features(data, monthly_data, is_training=True):
    """
    Prepare features optimized for Cluster 2 - emphasizing stability and economic indicators
    """
    features = pd.DataFrame(index=data.index)
    
    # Basic demand lags - for test data, we need to be careful about the boundary
    for lag in [30, 60, 90]:  # Ensure no data from the last month is used
        features[f'Demand_lag_{lag}d'] = data['Dispatched Quantity'].shift(lag)


    # Rolling statistics - using only past data
    for window in [30, 60, 90]:  # Rolling window skips last month
        roll = data['Dispatched Quantity'].shift(30).rolling(window=window, min_periods=1)
...
def train_model(X_train, y_train, X_val, y_val):
    """
    Train CatBoost model optimized for Cluster 2
    """
    model = CatBoostRegressor(
        loss_function='RMSE',
        iterations=100,              # Same as Cluster 0
        learning_rate=0.045,         # Slightly reduced to stabilize market trends
        depth=5,                     # Increased slightly to capture macro trends
        min_data_in_leaf=3,          # Slight increase for stability
        subsample=0.78,              # Slightly lower to reduce market noise sensitivity
        colsample_bylevel=0.78,      # Matched subsample to avoid unstable patterns
        random_seed=42,
        verbose=False
    )
...
def plot_monthly_forecast(monthly_df, material_id, material_no, output_path, metrics):
...
def main(material_id, material_no, output_path=None):
    ...
    try:
        ...
        full_date_range = pd.date_range(start='2022-01-01', end='2024-11-30', freq='D')
        ...
        # Split data
        train_cutoff = pd.to_datetime('2024-05-01')  # Shift train cutoff one month back
        val_cutoff = train_cutoff - pd.DateOffset(months=1)

        # Ensure test data starts at least one month later
        test_data = daily_df[daily_df.index >= (train_cutoff + pd.DateOffset(months=1))].copy()
        ...
        monthly_test_data = external_df[external_df.index >= (train_cutoff + pd.DateOffset(months=1))].copy()
        ...
        # Make predictions
        test_pred = model.predict(X_test.shift(30))  # Ensure no leakage from last month
        
        # Calculate daily metrics
        daily_metrics = calculate_metrics(y_test, test_pred)
        
        # Calculate weekly and monthly aggregates
        weekly_actual = y_test.shift(7).resample('W').sum().fillna(0)  # Shift by 7 days for weekly
        weekly_pred = pd.Series(test_pred, index=test_data.index).resample('W').sum().fillna(0)

        monthly_actual = y_test.shift(1).resample('MS').sum().fillna(0)  # Shift by 1 month for monthly
        monthly_pred = pd.Series(test_pred, index=test_data.index).resample('MS').sum().fillna(0)
