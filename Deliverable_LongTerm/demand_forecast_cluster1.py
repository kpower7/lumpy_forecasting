# %% [markdown]
# # Enhanced Demand Forecasting using CatBoost for Cluster 1 (Medium-Demand Economic Indicator Materials)
# This script is optimized for materials with medium average monthly demand, stable demand patterns, and strong positive correlation with economic indicators.

# %% [markdown]
# """
# Cluster 1: Medium-Demand Economic Indicator Materials
# 26 materials (31% of total)

# Characteristics:
# - Medium average monthly demand (510 units)
# - Most stable demand patterns (lowest CV = 0.52)
# - Moderate order fulfillment rate
# - Strong positive correlation with economic indicators:
#   * Positive PIB sensitivity (0.21)
#   * Strong ICI sensitivity (0.48)
#   * Mixed market sensitivity (ECG: -0.23, ISE: 0.29)

# This cluster represents materials with moderate, stable demand that show strong 
# positive correlation with economic indicators, particularly ICI. The mixed market
# sensitivity suggests these materials might be influenced more by broader economic
# conditions than specific market movements.

# Strategy:
# - Leverage economic indicators (especially ICI) for demand planning
# - Focus on maintaining stable inventory due to consistent demand
# - Monitor economic trends for proactive inventory adjustments
# """
...
def load_data(material_id: str, material_name: str) -> pd.DataFrame:
    """
    Load and prepare data for a specific material in Cluster 1 (Medium-Demand Economic Indicator)
    """
    try:
        # Load daily data
...
def prepare_features(data, monthly_data, is_training=True):
    """
    Prepare features optimized for Cluster 1 - emphasizing stability and economic indicators
    """
...
def train_model(X_train, y_train, X_val, y_val):
    """
    Train CatBoost model optimized for Cluster 1
    """
    model = CatBoostRegressor(
        loss_function='RMSE',
        iterations=100,              # Same as Cluster 0
        learning_rate=0.048,         # Slightly reduced to improve generalization
        depth=4,                     # Same as Cluster 0 to maintain stability
        min_data_in_leaf=2,          # Same as Cluster 0 to maintain stability
        subsample=0.82,              # Slightly higher to introduce more variability
        colsample_bylevel=0.82,      # Slightly higher to include more features
        random_seed=42,
        verbose=False
    )
