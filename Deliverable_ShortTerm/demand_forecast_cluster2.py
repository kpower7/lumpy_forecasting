...
def prepare_features(data, monthly_data, is_training=True):
    ...
    for lag in [5, 7, 14, 30, 60]:  # Removed longer lags that might leak across train/test boundary
        features[f'Demand_lag_{lag}d'] = data['Dispatched Quantity'].shift(lag)

    # Rolling statistics - using only past data
    for window in [5, 7, 14, 30, 60]:  # Reduced windows to avoid boundary issues
    ...
    for var in external_vars:
        if var in monthly_data.columns:
            for lag in range(1, 12):  # Reduced lag range to avoid leakage
                if not is_training:
...
def main(material_id, material_no, output_path=None):
    ...
    try:
        ...
        monthly_actual = y_test.resample('MS').sum()
        monthly_pred = pd.Series(test_pred, index=test_data.index).resample('MS').sum()
