# %% [markdown]
# # Enhanced Demand Forecasting using XGBoost-CatBoost Ensemble for Cluster 1
...
def main(material_id, material_no, output_path=None):
    ...
    try:
        ...
        ensemble_pred = predict_ensemble(xgb_model, cat_model, X_test)
        
        # Calculate daily metrics
        daily_metrics = calculate_metrics(y_test, ensemble_pred)
        
        # Calculate weekly and monthly aggregates
        weekly_actual = y_test.resample('W').sum()
        weekly_pred = pd.Series(ensemble_pred, index=test_data.index).resample('W').sum()
        
        monthly_actual = y_test.resample('ME').sum()
        monthly_pred = pd.Series(ensemble_pred, index=test_data.index).resample('ME').sum()
